...
const OPDCollectionReport = ({ navigation }) => {
  ...
  // Calculate totals. Number() guards against the driver handing back
  // DECIMAL columns as strings, which would concatenate instead of add.
  filteredRecords.forEach(item => {
    const amount = Number(item.total) || 0;
    item.payment_mode === 'Cash' && (totalCash += amount);
    item.payment_mode === 'Card' && (totalCard += amount);
    item.payment_mode === 'Online' && (totalOnline += amount);
    item.payment_mode === 'Cheque' && (totalCheque += amount);
  });
