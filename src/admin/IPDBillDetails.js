...
const IPDBillDetails = ({ navigation }) => {
  ...
  useEffect(() => {
    setPage(0);
    fetchIPDBills(location, fromDate, toDate);
  ...
  const fetchIPDBills = (location, from, to) => {
    ...
    try {
      setLoading(true);
      fetch(
        `${BACKEND_URL}/IPDCollection/billsV2?location=${location}&from=${from}&to=${to}&status=${''}`,
        requestOptions,
      )
        .then(response => response.json())
        .then(response => {
          console.log('Bill details: ', response.statusWiseTotals);
      ...
      >
        ...
        {isExpanded && (
          <View>
            ...
            <View style={styles.headerSubContainer}>
              {['Reimbursement', 'NonInsurance'].map(type => {
                const match = typewiseData.find(item => item.status === type);
                return (
                  <TouchableOpacity
                    key={type}
