...
const IVRCallList = ({ navigation }) => {
  ...
  const getData = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${BACKEND_URL}/IVRCall?location=${location}&from=${fromDate}&to=${toDate}&status=${status}`,
        {
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    const filtered = notAttended
      ? callRecord.filter(item => item.note === '')
      : callRecord;
    ...
    <SafeAreaView style={styles.maincontainer} edges={['top', 'bottom']}>
      ...
      >
        <View style={styles.headerContainer}>
          ...
          <Text style={styles.header}>IVR Call List</Text>
          <View
            style={{
              width: '25%',
              alignItems: 'center',
                ...
                {filteredList.slice(from, to).map((item, index) => (
                  <DataTable.Row
                    ...
                    style={{
                      ...
                      marginHorizontal: 3,
                      minHeight: 60,
                      backgroundColor: item.note ? '#FFB300' : '#fff',
                    }}
                  >
                    {/* Row content */}
                    <View
