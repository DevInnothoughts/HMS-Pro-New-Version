...
// Replaces src/admin/AddUserForm.js and the toggle-only editing in UserList.
//
...
// it — which loses the device lock and locks the person out until they
// re-verify. Every field is editable here.
//
// ⚠️ EDIT USES update(), NOT set()
// ────────────────────────────────
// set() REPLACES the document. The user doc also holds `otp` and `deviceId`,
// which this form never shows — set() would wipe both, signing the person out
// and destroying an OTP they may be mid-way through using. update() merges.
//
...
// claim, which is how someone gets locked out of their own account — so the
// two move together.
//
// ⚠️ mobile IS THE DOCUMENT ID
// ────────────────────────────
// Changing it means creating a new doc and deleting the old one, losing the
// roster link and the device lock in between. Read-only in edit; to move
// someone to a new number, add the new login and deactivate the old one.
// ─────────────────────────────────────────────────────────────────────────────
...
// The subRole values the app branches on. 'Owner' displays as "Partner" —
// ticketing/roles.js accepts both spellings.
const SUB_ROLES = [
  { value: '', label: 'None' },
...
const UserFormScreen = ({ navigation, route }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        // as "not allowed". Showing the stored value makes that visible.
  ...
  const validate = () => {
    ...
    // location rules below do not apply.
    if (needsDepartment) {
  ...
  const save = async () => {
    ...
    try {
      ...
      // with it. A stale deviceId is how someone gets locked out.
      ...
      // The roster is the second half of a ticketing account. Without it they
      // sign in to an empty queue, which looks like a bug rather than a missing
      // step — so a failure here is reported plainly.
      if (needsDepartment) {
        try {
          ...
          <View style={st.body}>
            ...
            {needsDepartment ? (
              ...
              <>
                ...
                <View style={st.card}>
                  <View style={[st.chips, { padding: 13, marginTop: 0 }]}>
