...
const PatientListScreen = ({ navigation }) => {
  ...
  const fetchLeadList = useCallback(location => {
    ...
    try {
      fetch(
        `${BACKEND_URL}/gpReferral/referredPatients?location=${location}&from=${fromDate}&to=${toDate}&role=${role}`,
        requestOptions,
      )
        ...
        .then(res => {
          ...
          setLeadStats({
            ...
            leads: res.leads,
            groupedReferred: res.groupedReferred,
          });
  ...
  const handleBillTypeSelect = type => {
    ...
    if (type === 'Appointment') {
      const phoneSet = new Set(
        leadStats.appointmentList.map(item => item.phoneno),
      );
      filtered = leadStats.leads.filter(item => phoneSet.has(item.phoneno));
    } else if (type === 'Visited') {
      const phoneSet = new Set(
        leadStats.visitedList.map(item => item.patient_phone),
      );
      filtered = leadStats.leads.filter(item => phoneSet.has(item.phoneno));
    } else if (type === 'IPD') {
      const phoneSet = new Set(
        leadStats.ipdList.map(item => item.patient_phone),
      );
      filtered = leadStats.leads.filter(item => phoneSet.has(item.phoneno));
    } else {
          ...
          <Card style={{ ...styles.cardTotal, flexDirection: 'column' }}>
            ...
            >
              <View style={{ minWidth: 130, flexDirection: 'column' }}>
                ...
                >
                  <Text style={styles.header}> Referred Patients : </Text>
          ...
          {paginatedLeads.length > 0 ? (
            paginatedLeads.slice(from, to).map((item, index) => (
              ...
              >
                ...
                >
                  <TouchableOpacity
                    onPress={() => toggleExpandPatient(item.id)}
                    activeOpacity={0.9}
                    style={{
                      width: '100%',
                      minHeight: 40,
                  ...
                  >
                    ...
                    <View style={{ width: '25%' }}>
                      <Text style={styles.medium}>{item.patientsContact}</Text>
                    </View>
                    <View style={{ width: '10%' }}>
                      <Image
                        style={{
                    ...
                    >
                      <View style={styles.amountContainer}>
                        <Text style={styles.medium}>
                          Referred By:{' '}
                          <Text style={styles.cell}>
                            {item.doctorsName} ({item.doctorsContact})
                          </Text>
                        </Text>
                      </View>
                      <View style={styles.amountContainer}>
                        <Text style={styles.medium}>
                          Disease:{' '}
                          <Text style={styles.cell}>
                            {item.disease !== 'Null' ? item.disease : ''}
                          </Text>
                        </Text>
                      </View>
                      <View style={styles.amountContainer}>
                      ...
                      >
                        ...
                        >
                          <TouchableOpacity
                            activeOpacity={0.5}
                            onPress={() =>
                              handlePhoneCall(item.patientsContact)
                            }
                            style={{
                              display: 'flex',
                              justifyContent: 'center',
                        ...
                        >
                          <TouchableOpacity
                            activeOpacity={0.5}
                            onPress={() => handleWhatsapp(item.patientsContact)}
                            style={{
                              display: 'flex',
                              justifyContent: 'center',
                              alignItems: 'center',
